...
class ModernButton(Button):
    def __init__(self, text='', **kwargs):
        super().__init__(
            ...
            text_color=UI_COLORS['light'],
            scale=(0.2, 0.06),
            **kwargs
        )
        self.highlight_color = UI_COLORS['secondary']
        self.pressed_color = color.darken(UI_COLORS['primary'], 0.3)
        
    def on_mouse_enter(self):
        self.animate_scale((0.21, 0.062), duration=0.1)
        
    def on_mouse_exit(self):
        self.animate_scale((0.2, 0.06), duration=0.1)

class ProgressBar(Entity):
    def __init__(self, value=100, max_value=100, color=color.green, position=(-0.8, 0.45), **kwargs):
...
class IconDisplay(Entity):
    def __init__(self, icon='●', color=color.white, position=(0, 0), scale=1, **kwargs):
        super().__init__(parent=camera.ui, **kwargs)
        self.icon = Text(
            text=icon,
            position=position,
...
class MiniMap(Entity):
    def __init__(self, position=(0.75, 0.35), size=0.2, **kwargs):
        ...
        for i in range(-5, 6):
            line = Entity(
                ...
                scale=(0.002, size * 0.9),
                position=(i * 0.1, 0, -0.01)
            )
        ...
        for i in range(-5, 6):
            line = Entity(
                ...
                model='quad',
                color=color.rgba(255, 255, 255, 50),
                scale=(size * 0.9, 0.002),
                position=(0, i * 0.1, -0.01)
            )
            self.grid_lines.append(line)
    ...
    def update(self):
        ...
        if player:
            # Simple mapping of player position to minimap
...
class NotificationSystem:
    ...
    def add_notification(self, title, message, icon='ℹ️', color=UI_COLORS['light']):
        notification = {
            'title': title,
            'message': message,
            'icon': icon,
...
class WeaponWheel(Entity):
    def __init__(self, **kwargs):
        ...
        self.weapons = [
            {'name': 'Fist', 'icon': '👊', 'ammo': '∞'},
            {'name': 'Pistol', 'icon': '🔫', 'ammo': '30'},
            {'name': 'Shotgun', 'icon': '💥', 'ammo': '8'},
            {'name': 'SMG', 'icon': '🔫', 'ammo': '60'},
            {'name': 'Assault Rifle', 'icon': '🔫', 'ammo': '90'},
            {'name': 'Sniper', 'icon': '🎯', 'ammo': '10'},
            {'name': 'RPG', 'icon': '🚀', 'ammo': '3'},
            {'name': 'Grenade', 'icon': '💣', 'ammo': '5'}
        ]
        ...
        # Weapon slots
        self.slots = []
        angle_step = 360 / len(self.weapons)
        for i, weapon in enumerate(self.weapons):
            ...
            slot = Entity(
                parent=self.bg,
                model='circle',
                color=color.rgba(50, 50, 50, 200) if i != self.selected_index else UI_COLORS['primary'],
                scale=0.08,
                position=(x, y)
            )
            ...
            self.slots.append({'entity': slot, 'icon': icon, 'index': i})
            
        # Center display
        self.center_bg = Entity(
            parent=self.bg,
            model='circle',
        ...
        self.selected_icon = Text(
            parent=self.center_bg,
            text=self.weapons[self.selected_index]['icon'],
            scale=3
        )
        
        self.selected_name = Text(
            parent=self.center_bg,
            text=self.weapons[self.selected_index]['name'],
            position=(0, -0.15),
            scale=1.2,
            color=color.white
        )
        ...
        self.ammo_display = Text(
            parent=self.center_bg,
            text=f"Ammo: {self.weapons[self.selected_index]['ammo']}",
            position=(0, -0.25),
            scale=1,
            color=color.gray
        )
    ...
    def toggle(self):
        ...
        if self.visible:
            self.bg.animate_scale(0.5, duration=0.2)
            mouse.locked = False
        else:
            self.bg.animate_scale(0, duration=0.2)
            mouse.locked = True
    ...
    def select_weapon(self, index):
        ...
        for slot in self.slots:
            if slot['index'] == index:
                slot['entity'].color = UI_COLORS['primary']
            else:
                slot['entity'].color = color.rgba(50, 50, 50, 200)
                
        self.selected_icon.text = self.weapons[index]['icon']
        self.selected_name.text = self.weapons[index]['name']
        self.ammo_display.text = f"Ammo: {self.weapons[index]['ammo']}"
        
        # Update game state
        game_state['weapon'] = self.weapons[index]['name']
...
class MissionDisplay(Entity):
    ...
    def set_mission(self, name, objective):
        self.active = True
        self.name.text = name
...
class PauseMenu(Entity):
    def __init__(self, **kwargs):
        ...
        self.missions_btn = ModernButton(
            parent=self.panel,
            text='MISSIONS',
            position=(0, 0),
            on_click=self.show_missions
        )
...
def update_ui():
    ...
    money_display.update_value(game_state['money'])
    ammo_display.update_value(game_state['ammo'], game_state['max_ammo'])
    weapon_name.text = game_state['weapon']
...
def start_test_mission():
    mission_display.set_mission(
        'First Assignment',
        'Steal a car and deliver it to the garage'
    )
...
# ------------------------------------------------------------
# GAME CODE (Same as before but with UI integration)
# ------------------------------------------------------------
...
def update():
    ...
    game_state['time'] = f'{int((time.time() % 1440)/60):02d}:{int(time.time() % 60):02d}'
